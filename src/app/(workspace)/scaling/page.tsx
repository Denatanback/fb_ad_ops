...
export default async function ScalingPage({ searchParams }: ScalingPageProps) {
  const values = {
  ...
  const [data, approaches] = await Promise.all([getOperationalViewData("scaling", filters), listApproachOptions()]);

  return <OperationalWorkspaceView approaches={approaches} data={data} values={values} view="scaling" />;
}
