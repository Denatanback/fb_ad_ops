...
export default async function QueuePage({ searchParams }: QueuePageProps) {
  const values = {
    ...
    sort: parseOperationalSortValue(searchParams?.sort ?? null) ?? "recent"
  };
  ...
  const [data, approaches] = await Promise.all([getOperationalViewData("queue", filters), listApproachOptions()]);

  return <OperationalWorkspaceView approaches={approaches} data={data} values={values} view="queue" />;
}
